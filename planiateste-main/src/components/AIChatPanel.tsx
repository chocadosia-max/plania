  ...
  return (
    <div className="flex justify-start" style={{ animation: "reveal 0.4s cubic-bezier(0.16,1,0.3,1) both" }}>
      <div className="max-w-[90%]">
        <div className="bg-muted text-foreground rounded-2xl rounded-bl-md px-3.5 py-2.5 text-sm leading-relaxed whitespace-pre-line">
          {text}
...
export function AIChatPanel() {
  ...
  return (
    <>
      ...
      {open && (
        <div
          className={`fixed top-0 right-0 z-50 w-full sm:w-[400px] h-full glass-card shadow-2xl flex flex-col border-l border-border/30 ${
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 py-4 border-b border-border/30">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-foreground">PlanIA · Assistente Financeiro</p>
            ...
            {messages.map((msg) =>
              msg.role === "user" ? (
                <div key={msg.id} className="flex justify-end" style={{ animation: "reveal 0.3s cubic-bezier(0.16,1,0.3,1) both" }}>
                  <div className="max-w-[85%] bg-primary text-primary-foreground rounded-2xl rounded-br-md px-3.5 py-2.5 text-sm leading-relaxed">
                    {msg.text}
                  </div>
                </div>
              ) : (
          ...
          {showSuggestions && (
            <div className="px-4 pb-2">
              <p className="text-[10px] text-muted-foreground mb-2 uppercase tracking-wider font-medium">Sugestões</p>
              <div className="flex flex-wrap gap-1.5">
                {suggestedQuestions.map((q) => (
                  <button
                    key={q}
                    onClick={() => send(q)}
                    className="text-[11px] px-3 py-1.5 rounded-full border border-border/60 text-muted-foreground hover:text-foreground hover:border-primary/40 hover:bg-primary/5 transition-all duration-200 active:scale-[0.96]"
                  >
                    {q}
                  </button>
                ))}
